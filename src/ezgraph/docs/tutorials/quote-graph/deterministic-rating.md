...
`CoverageNode` reads the committed vehicle-use channel, validates the requested coverage against
ownership and the quote clock, then builds the rating subject from the registered node channels.

```ts
const error = validateCoverageSelection(coverage, use.ownership, now);
if (error) return stay(JSON.stringify({ accepted: false, error }));

const rating = buildRatingSubject(state.nodes);
if ("error" in rating) return stay(JSON.stringify({ accepted: false, error: rating.error }));
